...
async function criarUsuario(email: string, role: 'agricultor' | 'instituicao', perfilId: string) {
  const senha = await bcrypt.hash(SENHA_PADRAO, BCRYPT_ROUNDS);
...
async function seed() {
  ...
  const escola = await InstituicaoRepository.save(InstituicaoRepository.create({
  ...
  const ifto = await InstituicaoRepository.save(InstituicaoRepository.create({
  ...
  const joao = await AgricultorRepository.save(AgricultorRepository.create({
    ...
    realizaEntrega: true,
    produtos: [
      { nome: 'Alface Americana', categoria: 'Hortalicas', capacidadeMensal: 500, unidade: 'kg', mesesDisponiveis: ['Jan','Fev','Mar','Abr','Mai','Jun'], organico: true, precoSugerido: 4.5 },
      { nome: 'Tomate Cereja', categoria: 'Hortalicas', capacidadeMensal: 300, unidade: 'kg', mesesDisponiveis: ['Jan','Fev','Mar','Nov','Dez'], organico: false, precoSugerido: 8.0 },
    ] as any,
  }));
  await criarUsuario(joao.email, 'agricultor', joao.id);

  const maria = await AgricultorRepository.save(AgricultorRepository.create({
    ...
    produtos: [
      { nome: 'Feijao Carioca', categoria: 'Leguminosas', capacidadeMensal: 800, unidade: 'kg', mesesDisponiveis: ['Mar','Abr','Mai','Jun','Jul','Ago'], organico: false, precoSugerido: 7.0 },
    ] as any,
  }));
  await criarUsuario(maria.email, 'agricultor', maria.id);

  const chamada = await ChamadaRepository.save(ChamadaRepository.create({
    titulo: 'Chamada Publica no 01/2026 - Merenda Escolar',
    instituicaoId: escola.id,
    descricao: 'Chamada publica para aquisicao de generos alimenticios da agricultura familiar para a merenda escolar.',
  ...
  await PropostaRepository.save(PropostaRepository.create({
  ...
  console.log('\nSeed concluido! Usuarios de exemplo (senha: senha123):');
  console.log(`  Instituicao: ${escola.email}`);
  console.log(`  Instituicao: ${ifto.email}`);
  console.log(`  Agricultor:  ${joao.email}`);
  console.log(`  Agricultor:  ${maria.email}`);
