...
export function PerfilInstituicaoView({ id, nav }: { id: string; nav: Nav }) {
  ...
  return <View className="flex-1 bg-agro-bg"><Header title="Perfil da Instituicao" onBack={nav.back} /><KeyboardAwareScrollView keyboardShouldPersistTaps="handled" contentContainerClassName="gap-3 p-4 pb-7"><InstituicaoProfile instituicao={getInstituicao(id)} chamadas={getChamadasByInstituicao(id)} isOwner={role === 'instituicao' && id === currentUserId} nav={nav} /></KeyboardAwareScrollView></View>;
}
