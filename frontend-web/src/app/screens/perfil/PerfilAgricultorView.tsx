...
export function PerfilAgricultorView({ id, nav }: { id: string; nav: Nav }) {
  ...
  return <View className="flex-1 bg-agro-bg"><Header title="Perfil do Agricultor" onBack={nav.back} /><ScrollView contentContainerClassName="gap-3 p-4 pb-7"><AgricultorProfile agricultor={getAgricultor(id)} isOwner={role === 'agricultor' && id === currentUserId} nav={nav} /></ScrollView></View>;
}
