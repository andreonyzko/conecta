...
export function EditarInstituicao({ nav }: { nav: Nav }) {
  ...
  return (
    <View className="flex-1 bg-agro-bg">
      ...
      <View className="gap-2.5 border-t border-agro-border bg-agro-bg px-3.5 pb-8 pt-3.5"><Button grow={false} onPress={() => { if (!nome.trim()) return notify('Informe o nome.'); updateInstituicao({ ...inst, nome, cnpj, telefone, email, numeroAlunos: Number(numeroAlunos) }); notify('Perfil atualizado com sucesso.'); nav.go({ name: 'perfil' }, true); }}>Salvar Alteracoes</Button></View>
    </View>
  );
}
