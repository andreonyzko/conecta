...
export function CriarChamada({ nav }: { nav: Nav }) {
  ...
  return (
    <View className="flex-1 bg-agro-bg">
      ...
      <View className="gap-2.5 border-t border-agro-border bg-agro-bg px-3.5 pb-8 pt-3.5"><Button grow={false} onPress={() => { if (!titulo.trim()) return notify('Informe o titulo da chamada.'); if (!dataInicio || !dataFim) return notify('Informe as datas da chamada.'); if (!itens.length) return notify('Adicione ao menos um item.'); addChamada({ titulo, descricao, dataInicio, dataFim, itens, instituicaoId: currentUserId, status: 'ativa' }); notify('Chamada publicada com sucesso.'); nav.go({ name: 'chamadas' }, true); }}>Publicar Chamada</Button></View>
