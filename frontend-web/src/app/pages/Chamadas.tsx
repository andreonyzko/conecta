...
import { Bell, Search, Calendar, ChevronRight, School, Leaf } from 'lucide-react';
...
function ChamadaCard({ chamada, instNome }: { chamada: Chamada; instNome: string }) {
  ...
  return (
    <div className="bg-[#1D2226] border border-[#2F3336] rounded-2xl p-4 mb-3 active:border-[#149D7F]/40 transition-colors">
...
export function Chamadas() {
  ...
  const list = (activeTab === 'todas' ? chamadas : minhasChamadas).filter((c) =>
      ...
      <div className="bg-[#1D2226] border-b border-[#2F3336] px-4 pt-3 pb-0 flex-shrink-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Leaf size={20} className="text-[#149D7F]" />
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-9 h-9 flex items-center justify-center text-[#B0B3B8] rounded-full active:bg-[#2F3336]"
            >
              <Search size={20} />
            </button>
            <button className="w-9 h-9 flex items-center justify-center text-[#B0B3B8] rounded-full active:bg-[#2F3336]">
              <Bell size={20} />
            </button>
          </div>
        </div>
