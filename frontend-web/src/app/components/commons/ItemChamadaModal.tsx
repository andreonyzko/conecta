...
export function ItemChamadaModal({ visible, initial, onClose, onSave }: { visible: boolean; initial?: ItemChamada; onClose: () => void; onSave: (item: ItemChamada) => void }) {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View className="flex-1 justify-end bg-black/60"><View className="max-h-[88%] overflow-hidden rounded-t-[22px] bg-agro-panel"><Header title={initial ? 'Editar Item' : 'Novo Item'} onBack={onClose} /><KeyboardAwareScrollView keyboardShouldPersistTaps="handled" contentContainerClassName="gap-3 p-4 pb-7">
