...
export function ProdutoModal({ visible, initial, onClose, onSave }: { visible: boolean; initial?: ProdutoAgricultor; onClose: () => void; onSave: (produto: ProdutoAgricultor) => void }) {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View className="flex-1 justify-end bg-black/60"><View className="max-h-[88%] overflow-hidden rounded-t-[22px] bg-agro-panel"><Header title={initial ? 'Editar Produto' : 'Novo Produto'} onBack={onClose} /><KeyboardAwareScrollView keyboardShouldPersistTaps="handled" contentContainerClassName="gap-3 p-4 pb-7">
