  ...
  return (
    <>
      <View className="gap-2.5 rounded-[18px] border border-agro-border bg-agro-panel p-4">
        <View className="flex-row items-center justify-between"><View className="h-[66px] w-[66px] items-center justify-center rounded-full bg-agro-green/15"><Text className="text-[21px] font-extrabold text-agro-green">{initials}</Text></View>{isOwner && <Button variant="outline" onPress={() => nav.go({ name: 'editarPerfil' })}>Editar</Button>}</View>
      ...
      {agricultor.avaliacoes.map((a) => <View key={a.id} className="gap-2.5 rounded-[18px] border border-agro-border bg-agro-panel p-4"><Text className="text-sm font-bold text-white">{getInstituicao(a.instituicaoId)?.nome || 'Instituicao'} · ★ {a.nota}</Text><Text className="text-xs text-agro-muted">{formatDate(a.data)}</Text><Text className="text-sm leading-5 text-white">{a.comentario}</Text></View>)}
